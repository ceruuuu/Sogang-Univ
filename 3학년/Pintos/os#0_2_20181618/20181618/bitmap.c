...
/* 비트맵의 길이를 size만큼 추가로 늘려 다시 return해주는 함수 */
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size){
	
	int before = bitmap_size(bitmap);
	int after = before + size;

	if(bitmap != NULL){
		int	before = bitmap_size(bitmap);
		struct bitmap *exp_bitmap = bitmap_create(after);
		bitmap_set_all(exp_bitmap, false);
		for(int i=0; i<before;i++){
			bitmap_set(exp_bitmap,i, bitmap_test(bitmap,i));
		}
		return exp_bitmap;
	}
	return NULL;
